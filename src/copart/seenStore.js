import {
  initRedis,
  closeRedis,
  addToSet,
  isInSet,
  getSetMembers,
  getSetSize,
  removeFromSet,
  clearSet,
  addMultipleToSet,
} from '../utils/redisStore.js';

const SET_KEY = 'copart:seen:lots';

// Инициализируем Redis при первом импорте
let redisInitialized = false;

async function ensureRedis() {
  if (!redisInitialized) {
    try {
      await initRedis();
      redisInitialized = true;
    } catch (error) {
      console.error('[seenStore] Ошибка инициализации Redis, продолжаем без Redis:', error.message || error);
      redisInitialized = true; // Помечаем как инициализированный, чтобы не пытаться снова
    }
  }
}

/**
 * Загружает все отправленные лоты из Redis
 * @returns {Promise<Set<string>>}
 */
export async function loadSeen() {
  try {
    await ensureRedis();
    return await getSetMembers(SET_KEY);
  } catch (error) {
    console.error('[seenStore] Ошибка загрузки из Redis, возвращаем пустой Set:', error.message || error);
    return new Set();
  }
}

/**
 * Сохраняет Set отправленных лотов в Redis
 * @param {Set<string>} seenSet
 */
export async function saveSeen(seenSet) {
  await ensureRedis();
  if (seenSet.size === 0) {
    await clearSet(SET_KEY);
    return;
  }
  await addMultipleToSet(SET_KEY, Array.from(seenSet));
}

/**
 * Проверяет, был ли лот уже отправлен
 * @param {string} lotId
 * @returns {Promise<boolean>}
 */
export async function hasSeen(lotId) {
  await ensureRedis();
  return await isInSet(SET_KEY, lotId);
}

/**
 * Добавляет лот в список отправленных
 * @param {string} lotId
 */
export async function addSeen(lotId) {
  try {
    await ensureRedis();
    await addToSet(SET_KEY, lotId);
  } catch (error) {
    console.error(`[seenStore] Ошибка добавления в Redis (${lotId}):`, error.message || error);
    // Не выбрасываем ошибку, чтобы приложение продолжало работу
  }
}

/**
 * Получает количество отправленных лотов
 * @returns {Promise<number>}
 */
export async function getSeenCount() {
  await ensureRedis();
  return await getSetSize(SET_KEY);
}

/**
 * Очищает список отправленных лотов
 */
export async function clearSeen() {
  await ensureRedis();
  await clearSet(SET_KEY);
}

/**
 * Закрывает соединение с Redis (для graceful shutdown)
 */
export async function close() {
  await closeRedis();
  redisInitialized = false;
}
